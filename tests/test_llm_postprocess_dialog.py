...
class LLMPostprocessDialogTests(unittest.TestCase):
    ...
    def test_default_output_save_path_prefers_loaded_transcript_directory_and_name(self) -> None:
        ...
        try:
            dialog._loaded_transcript_path = r"C:\Users\pyscribe-user\Documents\meeting_notes.txt"
            dialog.template_combo.setCurrentIndex(0)
            self.assertEqual(
                dialog._default_output_save_path(),
                r"C:\Users\pyscribe-user\Documents\meeting_notes_postprocess_meeting-summary.md",
            )
        finally:
            dialog.close()
    ...
    def test_default_output_save_path_falls_back_to_last_save_dir(self) -> None:
        dialog = self._build_dialog()
        try:
            dialog._config.last_save_dir = r"C:\Users\pyscribe-user\Documents\Exports"
            dialog.template_combo.setCurrentIndex(0)
            self.assertEqual(
                dialog._default_output_save_path(),
                r"C:\Users\pyscribe-user\Documents\Exports\postprocess_meeting-summary.md",
            )
        finally:
            dialog.close()
