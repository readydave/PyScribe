...
"""
This module contains the core logic for transcribing audio using the faster-whisper library.
It handles model loading, audio preparation, language detection, transcription/translation, and UI updates.
"""
...
def run_transcription(app_instance, model_name: str, media_path: str):
    ...
    Raises:
        ...
        RuntimeError: If an ffmpeg error occurs during audio processing.
    """
    ...
    try:
        ...
        if app_instance.current_model_name != model_name:
            # If the requested model is different from the currently loaded one,
            # load the new model. This avoids reloading the same model multiple times.
            # The compute_type is set based on the available device (CUDA for float16, CPU for int8).
        ...
        # Convert the input media file to a 16kHz mono WAV format, which is required by Whisper.
        ...
        # Probe the audio file to get its duration for progress calculation.
        try:
        ...
        # Load the audio waveform into a NumPy array.
        ...
        # If an English-only model is not selected, attempt to detect the language.
        if not is_english_only_model:
            ...
            # If a non-English language is detected with high confidence,
            # prompt the user whether to translate to English or transcribe in the detected language.
            if detected_lang_code != 'en' and lang_prob > 0.6:
                app_instance.prompt_for_translation(detected_lang_code)
                # User's choice determines the task and language for transcription.
        ...
        # Step 4: Perform Transcription or Translation
        status_msg = "Translating..." if task == "translate" else f"Transcribing in {lang_code}..."
        app_instance.set_status(status_msg)
        
        # Start the transcription/translation process using the loaded model.
        # The 'language' parameter is set to None for translation tasks (to English).
        ...
        # Iterate through the generated segments, update the UI with partial results,
        ...
        for segment in segments_generator:
            ...
            if duration > 0:
                ...
                # Ensure progress doesn't exceed 100% due to floating point inaccuracies.
        ...
        # Enable save and copy buttons after successful transcription.
        app_instance.enable_save_buttons()

    except Exception as e:
        # Catch any exceptions during the process and display an error message.
        app_instance.show_error(f"An error occurred during transcription:\n\n{e}")
        app_instance.set_status("An error occurred.")
    finally:
        # Ensure UI state is reset and temporary directory is cleaned up.
